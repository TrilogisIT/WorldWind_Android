...
/**
 * WWEvent is the base class which all World Wind event objects derive from. It extends Java's base {@link java.util.EventObject} by adding the capability to
 * consume the event by calling {@link #consume()}. Consuming a
 * WWEvent prevents is from being processed in the default manner by the source that originated the event. If the event
 * cannot be consumed, calling {@code consume()} has no effect, though {@link #isConsumed()} returns whether or not {@code consume()} has been called.
 * 
 * @author dcollins
...
public class WWEvent extends EventObject {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -393214983244700821L;
	 ...
	 * <p/>
	 * Note: if the event cannot be consumed, this still returns {@code true} if {@link #consume()} has been called, though this has no effect.
	 * 
