 ...
 * <p/>
 * <code>KMLAbstractFeature</code> implements the <code>KMLRenderable</code> interface, but does not actually render anything. Subclasses should override the
 * methods <code>{@link #doPreRender(KMLTraversalContext,
 * gov.nasa.worldwind.render.DrawContext)}</code> and <code>{@link #doRender(KMLTraversalContext,
 * gov.nasa.worldwind.render.DrawContext)}</code> to render their contents. If the <code>visibility</code> property is set to <code>false</code>, this does not
 * call <code>doPreRender</code> and <code>doRender</code> during rendering.
 * 
 * @author tag
...
public class KMLAbstractFeature extends KMLAbstractObject implements KMLRenderable {
	 ...
	 * initially, but the client may set it directly, in which case it may then differ from the visibility field in the <code>fields</code> table.
	 */
	...
	protected void doAddEventContent(Object o, XMLEventParserContext ctx, XMLEvent event, Object... args) {
		if (o instanceof KMLAbstractView) this.setView((KMLAbstractView) o);
		else if (o instanceof KMLAbstractTimePrimitive) this.setTimePrimitive((KMLAbstractTimePrimitive) o);
		else if (o instanceof KMLAbstractStyleSelector) this.addStyleSelector((KMLAbstractStyleSelector) o);
		else if (o instanceof KMLRegion) this.setRegion((KMLRegion) o);
		else if (o instanceof Boolean && event.getName().getLocalPart().equalsIgnoreCase("visibility")) this.setVisibility((Boolean) o);
	...
	/**
	 * Indicates whether this <code>KMLAbstractFeature</code> is enabled for rendering. This returns <code>null</code> if no visibility is specified. This
	 * indicates the default visibility of <code>true</code> should be used.
	 * 
	 ...
	 * 
	 * @param visibility
	 *            <code>true</code> or <code>null</code> to draw this feature, otherwise <code>false</code>. The
	...
	public String getSnippetText() {
		...
		if (snippet != null && snippet.getCharacters() != null) return snippet.getCharacters().trim(); // trim because string parser might not have parsed it
	...
	public void preRender(KMLTraversalContext tc, DrawContext dc) {
		...
		if (!this.isFeatureActive(tc, dc)) return;
	...
	/**
	 * Indicates whether this <code>KMLAbstractFeature</code> is active and should be rendered on the specified <code>DrawContext</code>. This returns
	 * <code>true</code> if the following conditions are all <code>true</code>:
	 * <p/>
	 * <ul>
	 * <li>This feature's <code>visibility</code> is unspecified (<code>null</code>) or is set to <code>true</code>.</li>
	 * <li>This feature as no Region and does not inherit a Region from an ancestor, or its Region is active for the specified <code>DrawContext</code>.</li>
	 * </ul>
	 * <p/>
	 * If this feature has no Region, this inherits the Region of its nearest ancestor by using the Region on the top of the KML traversal context's region
	 * stack (if any). If there is no ancestor Region this feature is assumed to be the
	 * <code>DrawContext's</cod> view and is rendered according to its <code>visibility</code> flag. A Region is considered active if it is visible, and the
	 * <code>DrawContext</code> meets the Region's level of detail criteria.
	 * 
	 * @param tc
	 *            the current KML traversal context. Specifies an inherited Region (if any) and a detail hint.
	 * @param dc
	 *            the current draw context. Used to determine whether this feature's Region is active.
	...
	protected boolean isFeatureActive(KMLTraversalContext tc, DrawContext dc) {
		if (this.getVisibility() != null && !this.getVisibility()) return false;

		KMLRegion region = this.getRegion();
		if (region == null) region = tc.peekRegion();
	 ...
	 * 
	 * @param tc
	 *            the current KML traversal context.
	 * @param dc
	...
	/**
	 * Called from <code>render</code> if this <code>KMLAbstractFeature</code>'s <code>visibility</code> is not set to <code>false</code>. Subclasses should
	 * override this method to render their content.
	 * 
	 * @param tc
	 *            the current KML traversal context.
	 * @param dc
	 ...
	 * <p/>
	 * A remote <i>styleUrl</i> that has not yet been resolved is not included in the result. In this case the returned sub-style is marked with the value
	 * {@link gov.nasa.worldwind.avlist.AVKey#UNRESOLVED}. The same is true when a StyleMap style selector contains a reference to an external Style and that
	 * reference has not been resolved.
	 * 
	 * @param styleState
	 *            the style mode, either \"normal\" or \"highlight\".
	 * @param subStyle
	 *            an instance of the sub-style desired, such as {@link KMLIconStyle}.
	 ...
	 * @return the sub-style values for the specified type and state. The reference returned is the one passed in as the <code>subStyle</code> argument.
	 */
	public KMLAbstractSubStyle getSubStyle(KMLAbstractSubStyle subStyle, String styleState) {
		return KMLAbstractStyleSelector.mergeSubStyles(this.getStyleUrl(), this.getStyleSelectors(), styleState, subStyle);
	}
	...
	public void applyChange(KMLAbstractObject sourceValues) {
		...
		if (sourceValues.hasField("visibility")) this.setVisibility((Boolean) sourceFeature.getField("visibility"));

		if (sourceFeature.getRegion() != null) this.setRegion(sourceFeature.getRegion());
	...
	protected void mergeStyleSelectors(KMLAbstractFeature sourceFeature) {
		...
		List<KMLAbstractStyleSelector> styleSelectorsCopy = new ArrayList<KMLAbstractStyleSelector>(this.getStyleSelectors().size());
