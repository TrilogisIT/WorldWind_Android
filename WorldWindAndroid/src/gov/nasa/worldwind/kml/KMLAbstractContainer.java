...
public class KMLAbstractContainer extends KMLAbstractFeature {
	...
	protected void doAddEventContent(Object o, XMLEventParserContext ctx, XMLEvent event, Object... args) {
		if (o instanceof KMLAbstractFeature) this.addFeature((KMLAbstractFeature) o);
	...
	public void removeFeature(KMLAbstractFeature feature) {
		if (feature != null) this.getFeatures().remove(feature);
	}

	/**
	 * Indicates whether this <code>KMLAbstractContainer</code> is active and should be rendered on the specified <code>DrawContext</code>. This returns
	 * <code>true</code> if this container's <code>visibility</code> is
	 * unspecified (<code>null</code>) or is set to <code>true</code>.
	 * <p/>
	 * Regions do not apply directly to KML containers, because a descendant features can override the container's Region with its own. Since a descendant
	 * Region may be larger or have a less restrictive LOD range than this container, we cannot determine the visibility of the entire tree based on this
	 * container's Region. A container's Region is inherited by any descendants that do not specify a Region, and Region visibility is determined at the leaf
	 * nodes.
	 * 
	 * @param tc
	 *            the current KML traversal context. This parameter is ignored.
	 * @param dc
	 *            the draw context. This parameter is ignored.
	 ...
	 * <p/>
	 * Pushes this container's Region on the KML traversal context's region stack before rendering the features, and pops the Region off the stack afterward.
	 * Descendant features use the KML traversal context's region stack to inherit Regions from parent containers.
	 * 
	 * @param tc
	...
	protected void beginRendering(KMLTraversalContext tc, DrawContext dc) {
		if (this.getRegion() != null) tc.pushRegion(this.getRegion());
	}

	/**
	 * Restores any rendering state changed during rendering. This pops this container's Region off the KML traversal
	 * context's region stack to restore the previous container's Region (if any). This must be preceded by a call to <code>beginRendering</code>.
	 * 
	 * @param tc
	 *            the current KML traversal context.
	 * @param dc
	...
	protected void endRendering(KMLTraversalContext tc, DrawContext dc) {
		if (this.getRegion() != null) tc.popRegion();
	}
	...
	protected void preRenderFeatures(KMLTraversalContext tc, DrawContext dc) {
		...
		for (KMLAbstractFeature feature : this.getFeatures()) {
			if (feature instanceof KMLAbstractContainer) containers.add(feature);
			else feature.preRender(tc, dc);
		}
	...
	public void applyChange(KMLAbstractObject sourceValues) {
		...
		if (sourceContainer.getFeatures() != null && sourceContainer.getFeatures().size() > 0) this.mergeFeatures(sourceContainer);
	}
	 ...
	 *            the incoming container of features.
	 */
