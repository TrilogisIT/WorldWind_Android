...
public class EllipsoidalGlobe extends AbstractGlobe {
	protected class StateKey implements GlobeStateKey {
		...
		public boolean equals(Object o) {
			...
			if (this.globe != null ? !this.globe.equals(that.globe) : that.globe != null) return false;
			if (this.elevationModel != null ? !this.elevationModel.equals(that.elevationModel) : that.elevationModel != null) return false;
			if (this.tessellator != null ? !this.tessellator.equals(that.tessellator) : that.tessellator != null) return false;
			if (Double.compare(this.verticalExaggeration, that.verticalExaggeration) != 0) return false;
	...
	public EllipsoidalGlobe(double equatorialRadius, double polarRadius, double es, ElevationModel elevationModel, Vec4 center) {
		super(elevationModel);
	...
	public boolean getIntersectionPosition(Line line, Position result) {
		...
		if (intersection == null || intersection.length == 0) return false;
	...
	/*
	 * (non-Javadoc)
	 * @see gov.nasa.worldwind.globes.Globe#computePointFromLocation(gov.nasa.worldwind.geom.LatLon)
	 */
	public Vec4 computePointFromLocation(LatLon location) {
		...
		return this.geodeticToCartesian(location.latitude, location.longitude, 0);
	}
	...
	/** {@inheritDoc} */
	public void computePointsFromPositions(Sector sector, int numLat, int numLon, double[] metersElevation, Vec4[] result) {
		if (sector == null) {
	 ...
	 *            the latitudeRadians of the position, in radians.
	 * @param longitudeRadians
	 *            the longitudeRadians of the position, in radians.
	 * @param metersElevation
	 *            the number of meters above or below mean sea level.
	 * @param result
	 *            contains the Cartesian point corresponding to the input position after this method
	 *            returns.
	 */
	protected void geodeticToCartesian(double latitudeRadians, double longitudeRadians, double metersElevation, Vec4 result) {
	 ...
	 * <p/>
	 * The Y axis points to the north pole. The Z axis points to the intersection of the prime meridian and the equator, in the equatorial plane. The X axis
	 * completes a right-handed coordinate system, and is 90 degrees east of the Z axis and also in the equatorial plane. Sea level is at z = zero.
	 * 
	 * @param sector
