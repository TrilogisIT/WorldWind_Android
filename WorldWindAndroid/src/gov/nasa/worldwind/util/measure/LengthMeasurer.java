...
public class LengthMeasurer implements MeasurableLength
{
    ...
    /**
     * Sets the type of path used when subdividing long segments, one of AVKey.GREAT_CIRCLE, which draws segments
     * as a great circle, AVKey.LINEAR, which determines the intermediate positions between segments by
     * interpolating the segment endpoints, or AVKey.RHUMB_LINE, which draws segments as a line of constant heading.
     *
            ...
            if (arcLength > maxLength)
            {
                ...
                for (int j = 1; j < steps; j++)
                {
                    ...
                    if (followTerrain)
                        ...
                        elevation = pos1.elevation * (1 - s) + pos2.elevation * s;
                    // Add new position
