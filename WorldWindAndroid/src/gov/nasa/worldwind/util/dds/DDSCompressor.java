 ...
 * <p/>
 * Each compression method accepts a reference to a {@link DXTCompressionAttributes}. This compressor performs the appropriate actions according to the
 * attributes, such as building mip maps and converting the source image to a premultiplied alpha format. Edited By: Nicola Dorigatti, Trilogis
 * 
 * @author dcollins
...
public class DDSCompressor {
	 ...
	 * @throws IllegalArgumentException
	 *             if <code>image</code> is null, or if <code>image</code> has non power of two
	 *             dimensions.
	 */
	...
	/**
	 * Converts the specified <code>image</code> to DDS using the DXT1 codec, and otherwise according to the <code>attributes</code>.
	 * 
	 * @param image
	...
	/**
	 * Converts the specified <code>image</code> to DDS using the DXT3 codec, and otherwise according to the <code>attributes</code>.
	 * 
	 * @param image
	...
	protected java.nio.ByteBuffer doCompressImage(DXTCompressor compressor, Bitmap image, DXTCompressionAttributes attributes) {
