...
public class DDSTextureReader {
	...
	protected int getFormat(DDSHeader header) {
		if (header.getPixelFormat() == null) return 0;

		int fourcc = header.getPixelFormat().getFourCC();

		if (fourcc == DDSConstants.D3DFMT_DXT1) return GL_COMPRESSED_RGBA_S3TC_DXT1_EXT;
		else if (fourcc == DDSConstants.D3DFMT_DXT2 || fourcc == DDSConstants.D3DFMT_DXT3) return GL_COMPRESSED_RGBA_S3TC_DXT3_EXT;
		else if (fourcc == DDSConstants.D3DFMT_DXT4 || fourcc == DDSConstants.D3DFMT_DXT5) return GL_COMPRESSED_RGBA_S3TC_DXT5_EXT;
		else if (fourcc == DDSConstants.D3DFMT_ETC1) return GL_ETC1_RGB8_OES;
	...
	protected int getImageSize(DDSHeader header, int width, int height) {
		...
		if (fourcc == DDSConstants.D3DFMT_DXT1 || fourcc == DDSConstants.D3DFMT_ETC1) return (minWidth * minHeight) / 2;
		else if (fourcc == DDSConstants.D3DFMT_DXT2 || fourcc == DDSConstants.D3DFMT_DXT3) return minWidth * minHeight;
		else if (fourcc == DDSConstants.D3DFMT_DXT4 || fourcc == DDSConstants.D3DFMT_DXT5) return minWidth * minHeight;
