 ...
 * <p/>
 * Individual tiles are identified by the level, row and column of the tile within its containing level set. Edited By: Nicola Dorigatti, Trilogis
 * 
 * @author dcollins
...
public class Tile implements Cacheable {
	 ...
	 * 
	 * @param sector
	 *            the sector corresponding with the tile.
	 * @param level
	 *            the tile's level within a containing level set.
	 * @param row
	 *            the row index (0 origin) of the tile within the indicated level.
	 * @param column
	 *            the column index (0 origin) of the tile within the indicated level.
	 ...
	 *            the column index (0 origin) of the tile within the indicated level.
	 * @param cacheName
	 *            optional cache name to override the Level's cache name. May be null.
	 * @throws IllegalArgumentException
	 *             if <code>sector</code> or <code>level</code> is null.
	 */
	 ...
	 * 
	 * @param sector
	 *            the sector to create the tile for.
	 * @param level
	 *            the level to associate the tile with
	 * @throws IllegalArgumentException
	 *             if sector or level are null.
	 */
	...
	public long getSizeInBytes() {
		...
		if (this.sector != null) size += this.sector.getSizeInBytes();

		if (this.path != null) size += this.getPath().length();
	...
	public String getPath() {
		if (this.path == null) {
			...
			if (!this.level.isEmpty()) path += this.level.getFormatSuffix();
		}
	...
	public final String getCacheName() {
		if (this.cacheName != null) return this.cacheName;
	...
	public int compareTo(Tile tile) {
		...
		if (tile.getLevelNumber() == this.getLevelNumber() && tile.row == this.row && tile.column == this.column) return 0;

		if (this.getLevelNumber() < tile.getLevelNumber()) // Lower-res levels compare lower than higher-res
		return -1;
		if (this.getLevelNumber() > tile.getLevelNumber()) return 1;

		if (this.row < tile.row) return -1;
		if (this.row > tile.row) return 1;
	 ...
	 * 
	 * @param nextLevel
	 *            the level for the sub tiles.
	 * @return a four-element array that contains this tile's sub tiles.
	 * @throws IllegalArgumentException
	 *             if <code>nextLevel</code> is <code>null</code>.
	 */
	public Tile[] subdivide(Level nextLevel, MemoryCache cache, TileFactory factory) {
		...
		if (this.childKeys == null) this.childKeys = this.createChildKeys(nextLevel);

		int subRow = 2 * this.row;
		int subCol = 2 * this.column;
		Tile subTile = (Tile) cache.get(this.childKeys[0]);
		if (subTile != null) result[0] = subTile;
		else result[0] = factory.createTile(Sector.fromDegrees(p0, p1, t0, t1), nextLevel, subRow, subCol);

		subCol = 2 * this.column + 1;
		subTile = (Tile) cache.get(this.childKeys[1]);
		if (subTile != null) result[1] = subTile;
		else result[1] = factory.createTile(Sector.fromDegrees(p0, p1, t1, t2), nextLevel, subRow, subCol);

		subRow = 2 * this.row + 1;
		subCol = 2 * this.column;
		subTile = (Tile) cache.get(this.childKeys[2]);
		if (subTile != null) result[2] = subTile;
		else result[2] = factory.createTile(Sector.fromDegrees(p1, p2, t0, t1), nextLevel, subRow, subCol);

		subCol = 2 * this.column + 1;
		subTile = (Tile) cache.get(this.childKeys[3]);
		if (subTile != null) result[3] = subTile;
		else result[3] = factory.createTile(Sector.fromDegrees(p1, p2, t1, t2), nextLevel, subRow, subCol);
	...
	public static void createTilesForLevel(Level level, Sector sector, TileFactory factory, List<Tile> result, LatLon origin) {
		...
		Angle latOrigin = origin.latitude;
		Angle lonOrigin = origin.longitude;

		int firstRow = computeRow(deltaLat, sector.minLatitude, latOrigin);
		int lastRow = computeRow(deltaLat, sector.maxLatitude, latOrigin);
		int firstCol = computeColumn(deltaLon, sector.minLongitude, lonOrigin);
		int lastCol = computeColumn(deltaLon, sector.maxLongitude, lonOrigin);

		Angle firstRowLat = computeRowLatitude(firstRow, deltaLat, latOrigin);
		Angle firstRowLon = computeColumnLongitude(firstCol, deltaLon, lonOrigin);
	 ...
	 * @throws IllegalArgumentException
	 *             if <code>delta</code> is null or non-positive, or <code>latitude</code> is null,
	...
	public static int computeRow(Angle delta, Angle latitude, Angle origin) {
		...
		if ((latitude.degrees - origin.degrees) == 180d) row = row - 1;
	 ...
	 * @throws IllegalArgumentException
	 *             if <code>delta</code> is null or non-positive, or <code>longitude</code> is
	...
	public static int computeColumn(Angle delta, Angle longitude, Angle origin) {
		...
		if (gridLongitude < 0.0) gridLongitude = 360d + gridLongitude;
		...
		if ((longitude.degrees - origin.degrees) == 360d) col = col - 1;
	 ...
	 * 
	 * @param row
	 *            the row index of the row in question
	 * @param delta
	 *            the grid interval
	 * @param origin
	 *            the origin of the grid
	 ...
	 * @throws IllegalArgumentException
	 *             if the grid interval (<code>delta</code>) is null or zero or the row index is
	 *             negative.
	 */
	 ...
	 * @throws IllegalArgumentException
	 *             if the grid interval (<code>delta</code>) is null or zero or the column index is
	 *             negative.
	 */
