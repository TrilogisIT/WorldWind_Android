 ...
 * <p/>
 * This cache limits the amount of time an entry remains in the cache unused. The maximum unused time may be specified. Entries unused within the specified
 * duration are removed from the cache each time {@link #getEntry(gov.nasa.worldwind.globes.Globe)} is called. Edited By: Nicola Dorigatti, Trilogis
 * 
 * @author tag
...
public class ShapeDataCache implements Iterable<ShapeDataCache.ShapeDataCacheEntry> {
	public static class ShapeDataCacheEntry extends AVListImpl {
		 ...
		 * 
		 * @param dc
		 *            the draw context. Must contain a globe.
		 * @param minExpiryTime
		 *            the minimum expiration duration, in milliseconds.
		 * @param maxExpiryTime
		 *            the maximum expiration duration, in milliseconds.
		...
		public boolean isValid(DrawContext dc) {
			return this.verticalExaggeration == dc.getVerticalExaggeration() && (this.globeStateKey != null && this.globeStateKey.equals(dc.getGlobe().getGlobeStateKey(dc)));
		}
		 ...
		 * 
		 * @param isExpired
		 *            true to expire the entry, otherwise false.
		 */
	 ...
	 * 
	 * @param globe
	 *            the globe the entry is associated with.
