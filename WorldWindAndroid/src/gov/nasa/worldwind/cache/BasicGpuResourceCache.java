 ...
 * <p/>
 * When a resource is removed from the cache, and if it is a recognized OpenGL resource -- a texture, a list of vertex buffer IDs, a list of display list IDs,
...
public class BasicGpuResourceCache implements GpuResourceCache {
	...
	public GpuProgram getProgram(Object key) {
		...
		CacheEntry entry = (CacheEntry) this.resources.get(key);
		return entry != null && entry.resourceType.equals(PROGRAM) ? (GpuProgram) entry.resource : null;
	}
	...
	public GpuTexture getTexture(Object key) {
		...
		CacheEntry entry = (CacheEntry) this.resources.get(key);
		return entry != null && entry.resourceType.equals(TEXTURE) ? (GpuTexture) entry.resource : null;
	}
	...
	protected long computeEntrySize(CacheEntry entry) {
		if (entry.resource instanceof Cacheable) return ((Cacheable) entry.resource).getSizeInBytes();
