...
public interface View extends WWObject {
	...
	/**
	 * Returns the combined modelview-projection matrix. This matrix is computed during the most recent call to <code>apply</code>, by multiplying the modelview
	 * and projection matrices. The returned matrix is equivalent to
	 ...
	 * space defined by three sets of parallel 'clipping' planes. This value is computed during the most recent call to <code>apply</code>.
	 * 
	 ...
	 * performing visibility tests against world geometry. This frustum has the same shape as the frustum returned in <code>getFrustum</code>, but it has been
	 * transformed into model coordinates. This value is computed during the
	 ...
	 *             If the implementation supports field-of-view, and <code>fieldOfView</code> is
	 *             null.
	 */
	 ...
	 * 
	 * @param modelPoint
	 *            the model coordinate point to project.
	 * @param result
	 *            contains the point transformed to screen coordinates after this method returns. This value is
	 ...
	 * @throws IllegalArgumentException
	 *             if either the model point or the result are <code>null</code>.
	 */
	 ...
	 * 
	 * @param screenPoint
	 *            the screen coordinate point to project.
	 * @param result
	 *            contains the point transformed to model coordinates after this method returns. This value is
	 *            not modified if this returns <code>false</code>.
	 * @return <code>true</code> if the screen point is successfully transformed, and <code>false</code> otherwise.
	 * @throws IllegalArgumentException
	 *             if either the screen point or the result are <code>null</code>.
	 */
	 ...
	 * 
	 * @param point
	 *            the screen point to compute a ray for.
	 * @param result
	 *            contains the line in model coordinates after this method returns. This value is not modified if
	 ...
	 * 
	 * @param globe
	 *            the globe to use when translating screen coordinates to geographic coordinates.
	 * @param point
	 *            the screen point for which to compute the geographic position.
	 * @param result
	 *            contains the screen point's geographic position after this method returns. This value is not
	 *            modified if this returns <code>false</code>.
	 * @return <code>true</code> if the screen point corresponds to a position on the globe's ellipsoid, and <code>false</code> otherwise.
	 */
	 ...
	 * 
	 * @param distance
	 *            the distance in meters from the eye point. This value must be positive or zero, but is otherwise
	 *            unbounded.
