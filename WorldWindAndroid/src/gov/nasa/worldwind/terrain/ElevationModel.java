 ...
 * <p/>
 * An <code>ElevationModel</code> often approximates elevations at multiple levels of spatial resolution. For any given viewing position, the model determines
 * an appropriate target resolution. That target resolution may not be immediately achievable, however, because the corresponding elevation data might not be
 * locally available and must be retrieved from a remote location. When this is the case, the elevations returned for a sector represent the resolution
 * achievable with the data currently available. That resolution may not be the same as the target resolution. The achieved resolution is made available in the
 * interface.
 * <p/>
 * 
 * @author dcollins
...
public interface ElevationModel extends WWObject, Disposable {
	 ...
	 * 
	 * @param name
	 *            the name to give the elevation model.
	 */
	 ...
	 * 
	 * @param networkRetrievalEnabled
	 *            <code>true</code> if network retrieval is allowed, else <code>false</code>.
	 */
	 ...
	 * 
	 * @param expiryTime
	 *            the expiry time of any cached data, expressed as a number of milliseconds beyond the epoch.
	 ...
	 * <p/>
	 * The missing-data value is often specified by the metadata of the data set, in which case the elevation model automatically defines that value to be the
	 * missing-data signal. When the missing-data signal is not specified in the metadata, the application may specify it via this method.
	 * 
	 * @param flag
	 *            the missing-data signal value. The default is -{@link Double#MAX_VALUE}.
	 ...
	 * <p/>
	 * The elevation returned from this method is the best available in memory. If no elevation is in memory, the elevation model's minimum extreme elevation at
	 * the location is returned. Local disk caches are not consulted.
	 * 
	 * @param latitude
	 *            the latitude of the location in question.
	 * @param longitude
	 ...
	 * 
	 * @param latitude
	 *            the latitude of the location for which to return the elevation.
	 * @param longitude
	 *            the longitude of the location for which to return the elevation.
	 ...
	 *            the sector in question.
	 * @param numLat
	 *            the grid height in number of latitude locations.
	 * @param numLon
	 *            the grid width in number of longitude locations.
	 * @param targetResolution
	 *            the desired horizontal resolution, in radians, of the raster or other elevation sample
	 *            from which elevations are drawn. (To compute radians from a distance, divide the distance
	 *            by the radius of the globe, ensuring that both the distance and the radius are in the
	 ...
	 *         determined for all of the locations.
	 * @throws IllegalArgumentException
	 *             if the sector is null, if either numLat or numLon are less than one, if
