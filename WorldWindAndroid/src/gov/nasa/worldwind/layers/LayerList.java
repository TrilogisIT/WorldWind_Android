...
public class LayerList extends CopyOnWriteArrayList<Layer> implements WWObject {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -7432103093425501540L;
	...
	public boolean addAll(Collection<? extends Layer> layers) {
		...
		if (added) this.firePropertyChange(AVKey.LAYERS, copy, this);
	...
	public boolean addIfAbsent(Layer layer) {
		for (Layer l : this) {
			if (l.equals(layer)) return false;
		}
	...
	public int addAllAbsent(Collection<? extends Layer> layers) {
		for (Layer layer : layers) {
			if (!this.contains(layer)) layer.addPropertyChangeListener(this);
		}

		LayerList copy = makeShallowCopy(this);
		int numAdded = super.addAllAbsent(layers);
		if (numAdded > 0) this.firePropertyChange(AVKey.LAYERS, copy, this);
	...
	public Layer set(int index, Layer layer) {
		...
		if (oldLayer != null) oldLayer.removePropertyChangeListener(this);
	...
	public boolean remove(Object o) {
		for (Layer layer : this) {
			if (layer.equals(o)) layer.removePropertyChangeListener(this);
		}

		LayerList copy = makeShallowCopy(this);
		boolean removed = super.remove(o);
		if (removed) this.firePropertyChange(AVKey.LAYERS, copy, this);
	...
	public boolean retainAll(Collection<?> objects) {
		for (Layer layer : this) {
			if (!objects.contains(layer)) layer.removePropertyChangeListener(this);
		}
	...
	public void replaceAll(Collection<? extends Layer> layers) {
		...
		for (Layer layer : layers) {
			if (!this.contains(layer)) toDelete.add(layer);
			else toKeep.add(layer);
		}
		...
		for (Layer layer : layers) {
			if (!toKeep.contains(layer)) layer.addPropertyChangeListener(this);
	 ...
	 * 
	 * @param lists
	 *            an array containing the lists to aggregate. All members of the second and subsequent lists in the
	 *            array are added to the first list in the array.
	 * @return the aggregated list.
	 * @throws IllegalArgumentException
	 *             if the layer-lists array is null or empty.
	 */
