 ...
 */
public class FAOCompassLayer extends AbstractLayer {
	...
	protected String iconFilePath = "images/faocompass.png"; // TODO: make configurable
	...
	protected boolean showTilt = false;
	...
	protected void doPick(DrawContext dc, Point pickPoint) {
		...
		dc.addOrderedRenderable(this.orderedImage);
		//get compass center and compass size.
		Vec4 compassCenter=computeLocation(viewport, computeScale(viewport));
		double compassWidth = computeScale(viewport) *  this.getScaledIconWidth();
		double compassHeight = computeScale(viewport) * this.getScaledIconHeight();
		//compute compass bounds
		double pxmin = compassCenter.x /*- (compassWidth/2)*/;
		double pxmax = compassCenter.x + compassWidth/*(compassWidth/2)*/;
		double pymin = compassCenter.y /*- (compassHeight/2)*/;
		double pymax = compassCenter.y + compassHeight/*(compassHeight/2)*/;
		//compute real pick point (in ww-opengl coordinates)
		double realY = viewport.height-pickPoint.y;
		double realX = pickPoint.x;
		
		/*rx > pxmin && < pxMax
		ry > pymin && < pymax;*/
		if((realX>pxmin&&realX<pxmax)&&(realY>pymin&&realY<pymax)){
			dc.addPickedObject(new PickedObject(0, this));
		}
		
		
		/*if((compassCenter.x-compassWidth<pickPoint.x&&compassCenter.x+compassWidth>pickPoint.x)&&(viewport.height-compassCenter.y-compassHeight<pickPoint.y&&viewport.height-compassCenter.y+compassHeight>pickPoint.y)){
			dc.addPickedObject(new PickedObject(10, this));
