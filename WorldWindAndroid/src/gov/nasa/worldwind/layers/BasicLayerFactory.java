...
public class BasicLayerFactory extends BasicFactory {
	 ...
	 * <li>a {@link java.io.InputStream}</li>
	 * <li>{@link org.w3c.dom.Element}</li>
	 * <li>a {@link String} holding a file name, a name of a resource on the classpath, or a string representation of a URL</li>
	 * </ul>
	 * <p/>
	 * For tiled image layers, this maps the <code>serviceName</code> attribute of the <code>Layer/Service</code> element of the XML configuration file to the
	 * appropriate base tiled image layer type. Service types recognized are:
	 * <ul>
	 * <li>"WMS" for layers that draw their data from a WMS web service.</li>
	 * <li>"WWTileService" for layers that draw their data from a World Wind tile service.</li>
	 * <li>"Offline" for layers that draw their data only from the local cache.</li>
	 * </ul>
	 * 
	 * @param configSource
	 ...
	 *             if the configuration file name is null or an empty string.
	 ...
	 * <li>a {@link java.io.InputStream}</li>
	 * <li>{@link Element}</li>
	 * <li>a {@link String} holding a file name, a name of a resource on the classpath, or a string represenation of a URL</li>
	 * </ul>
	 * <p/>
	 * For tiled image layers, this maps the <code>serviceName</code> attribute of the <code>Layer/Service</code> element of the XML configuration file to the
	 * appropriate base tiled image layer type. Service types recognized are:
	 ...
	 * 
	 * @param domElement
	 *            an XML element describing the layers and/or layer lists.
	...
	protected Object doCreateFromElement(Element domElement) throws Exception {
		...
		if (elements != null && elements.size() > 0) return createLayerLists(elements);

		elements = WWXML.getElements(domElement, "./Layer", xpath);
		if (elements != null && elements.size() > 1) return createLayerList(elements);

		if (elements != null && elements.size() == 1) return this.createFromLayerDocument(elements.get(0));

		String localName = WWXML.getUnqualifiedName(domElement);
		if (localName != null && localName.equals("Layer")) return this.createFromLayerDocument(domElement);
	 ...
	 * elements.
	 * <p/>
	 * Any exceptions occurring during creation of the layer lists or their included layers are logged and not re-thrown. The layers associated with the
	 * exceptions are not included in the returned layer list.
	 * 
	 * @param elements
	 *            the XML elements describing the layer lists to create.
			...
			try {
				...
				if (!WWUtil.isEmpty(href)) {
					...
					if (o instanceof LayerList) {
						...
						Logging.warning(Logging.getMessage("LayerFactory.UnexpectedTypeForLayer", o.getClass().getName()));
					}
	 ...
	 * <p/>
	 * Any exceptions occurring during creation of the layers are logged and not re-thrown. The layers associated with the exceptions are not included in the
	 * returned layer list.
	 * 
	 * @param layerElements
	 *            the XML elements describing the layers to create.
	 ...
	 * 
	 * @param domElement
	 *            the XML element describing the layer to create.
	 * @return a new layer
	 * @throws WWUnrecognizedException
	...
	protected Layer createFromLayerDocument(Element domElement) {
		...
		String layerName = WWXML.getText(domElement, "@title", xpath);
		if (!WWUtil.isEmpty(className)) {
			...
			if (null != layer && !WWUtil.isEmpty(layerName)) {
	 ...
	 * @return a new layer
	 * @throws WWUnrecognizedException
	 *             if the service type given in the describing element is unrecognized.
	 */
	protected Layer createTiledImageLayer(Element domElement) {
		return createTiledImageLayer(domElement, null);
	}
